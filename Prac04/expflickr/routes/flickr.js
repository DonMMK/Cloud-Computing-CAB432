...
router.get('/:query/:number', (req, res) => {
    ...
    const flickReq = https.request(options, (flickRes) => {
        ...
        flickRes.on('end', function() {
            res.writeHead(flickRes.statusCode, {
...
const flickr = {
    ...
    api_key: "87cf08c11f4a5c3b2986a6cfecb0f943",
    format: "json",
    media: "photos",
    nojsoncallback: 1
};
...
function createFlickrOptions(query, number) {
    ...
    const str = 'method=' + flickr.method +
        ...
        '&media=' + flickr.media + '&nojsoncallback=' + flickr.nojsoncallback;
    options.path += str;
    return options;
}
...
function createPage(title, rsp) {
    ...
    const str = '<!DOCTYPE html>' +
        '<html><head><title>Flickr JSON</title></head>' + '<body>' +
        '<h1>' + title + '</h1>' +
